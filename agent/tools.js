...
// ── Zod schemas (validação em runtime dos args gerados pela IA) ───────────────
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          description: {
            type: 'string',
            description: 'Resumo detalhado do que precisa ser feito. CAMPO OBRIGATÓRIO: Sempre gere uma descrição útil, mesmo que curta, baseada no que o usuário disse.',
          },
          priority: {
            type: 'string',
...
function fixDueDate(dateStr) {
  ...
  console.log(`[fixDueDate] ${dateStr} → ${fixed}`);
  return fixed;
}
...
async function executeTaskUpdate(args, userId) {
  ...
  if (!UUID_RE.test(task_id)) {
    ...
    resolvedId = found.id;
    console.log(`[TaskUpdate] ID resolvido por busca: "${searchTerm}" → ${resolvedId} ("${found.title}")`);
  }

  if (rest.due_date) {
    rest.due_date = fixDueDate(rest.due_date);
    // Prazo mudou → reseta lembrete para poder disparar novamente
...
async function executeTaskDelete(args, userId) {
  ...
  if (!UUID_RE.test(parsed.task_id)) {
    ...
    deleteId = found.id;
    console.log(`[TaskDelete] ID resolvido por busca: "${searchTerm}" → ${deleteId}`);
  }
