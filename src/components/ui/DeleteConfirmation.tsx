...
const DeleteConfirmation: React.FC<DeleteConfirmationProps> = ({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <Portal>
          <div className="fixed inset-0 z-[10000] bg-[#37352f]/05" onClick={onCancel} />
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 20, x: '-50%' }}
            animate={{ opacity: 1, scale: 1, y: 0, x: '-50%' }}
            exit={{ opacity: 0, scale: 0.95, y: 10, x: '-50%' }}
            transition={{ type: "spring", stiffness: 450, damping: 30 }}
            className="fixed bottom-10 left-1/2 z-[10001] bg-white border border-[#e9e9e7] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.12)] w-[260px] overflow-hidden"
          >
