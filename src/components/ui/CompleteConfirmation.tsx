...
const CompleteConfirmation: React.FC<CompleteConfirmationProps> = ({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <Portal>
          <div className="fixed inset-0 z-[10000]" onClick={onCancel} />
          <motion.div
            initial={{ opacity: 0, y: 50, scale: 0.95, x: '-50%' }}
            animate={{ opacity: 1, y: 0, scale: 1, x: '-50%' }}
            exit={{ opacity: 0, y: 20, scale: 0.95, x: '-50%' }}
            transition={{ type: "spring", stiffness: 400, damping: 25 }}
            className="fixed bottom-8 left-1/2 z-[10001] bg-white border border-[#e9e9e7] rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.12)] flex flex-col p-5 w-[90vw] max-w-[320px]"
          >
            <div className="mb-5">
